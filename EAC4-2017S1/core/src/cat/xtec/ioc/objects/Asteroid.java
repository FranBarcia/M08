...
public class Asteroid extends Scrollable {
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        batch.draw(AssetManager.asteroid, position.x, position.y, this.getOriginX(), this.getOriginY(), width, height, this.getScaleX(), this.getScaleY(), this.getRotation());
    }
    ...
    // TODO Ex3 - creem el métode per veure si hi ha colisió entre asteroides i lasers
    public boolean collidesLaser(Laser laser) {
        if (position.x <= (laser.getX() + (laser.getWidth() / 2))) {
