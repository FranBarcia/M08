...
public class Spacecraft extends Actor {
    ...
    // TODO Exercici 2 - metode que, segons si el joc està en pausa o no, aplica o elimina les animacions
    public void spacecraftPaused(Boolean paused) {
        if (paused) {
            this.addAction(Actions.repeat(RepeatAction.FOREVER, Actions.sequence(Actions.fadeIn(0.25f),Actions.fadeOut(0.25f))));
        } else {
            resetAlphaValue();
    ...
    // TODO Exercici 2 - metode que reseteja el valor de alpha
    public void resetAlphaValue() {
        Color color = this.getColor();
        this.setColor(color.r, color.g, color.b,1);
    }

    public void generarLaser(Laser laser) {
        lasers.add(laser);
    }
